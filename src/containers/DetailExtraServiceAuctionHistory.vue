...
const { refinedToday, refinedYesterday, refinedWeek, refinedMonth } =
  dateFormatterUtil;
const { requestAuctionHistory, requestDetailAuctionHistory } = extraService;
const { responseAuctionHistoryCodec, responseDetailAuctionHistoryCodec } =
  extraServiceCodec;
...
/** 경매 내역 리스트 */
const auctionHistoryData: Ref<responseAuctionHistoryType[]> = ref([]);
const getAuctionHistoryData = async () => {
  ...
  try {
    const res = (await requestAuctionHistory(
      storeCode,
      requestData,
    )) as AxiosResponse;
    const typeError = runtimeCheck(responseAuctionHistoryCodec, res.data.data);

    if (res.data.resultCode === 200) {
      if (!typeError) {
        auctionHistoryData.value = res.data.data.historyList;
...
const getDetailAuctionHistoryData = async (id: number) => {
  try {
    const res = (await requestDetailAuctionHistory(
      storeCode,
      id,
    )) as AxiosResponse;
...
/** 경매 내역 엑셀 다운로드 */
const downloadExcel = () => {
  const searchUrl = `${endpoints.entertainmentAdmin.stores.basic}/${storeCode}/auction/histories/excels?from=${selectedDate.value[0]}&to=${selectedDate.value[1]}`;
  ...
  try {
    ...
    gatewayTokenApi({
      ...
      link.download = `경매 내역_${refinedToday()}.xlsx`;
...
const openDetailModal = (id: number) => {
  ...
  getDetailAuctionHistoryData(id);
};
...
const getCountValue = (price: number) => {
  if (price !== null && price !== undefined) {
    return `${price?.toLocaleString()}회`;
  }
  return '-';
};

const getBidState = (data: responseDetailAuctionHistoryType, index: number) => {
  if (data.bidList && data.bidList[index]) return true;
  return false;
};

const getTargetBidData = (index: number, type: string) => {
  if (type === 'code') {
    return detailAuctionHistoryData.value?.bidList[index].tableCode;
  }
  return detailAuctionHistoryData.value?.bidList[
    index
...
<template>
  <el-dialog
    ...
    width="70%"
  >
    <div class="detail-auction-history-modal">
      <div class="detail-auction-history-modal-card">
        <p class="ml-10">경매 정보</p>
        <el-card
          class="detail-auction-history-modal-card-box"
          shadow="hover"
        >
          <div class="detail-auction-history-modal-card">
            <p class="detail-auction-history-modal-card-title">경매 상태</p>
            <p class="detail-auction-history-modal-card-content">
              {{ detailAuctionHistoryData.statusName }}
            </p>
            <p class="detail-auction-history-modal-card-title">경매 시작</p>
            <p class="detail-auction-history-modal-card-content">
              {{ detailAuctionHistoryData.startDateTime }}
            </p>
            <p class="detail-auction-history-modal-card-title">경매 시간</p>
            <p class="detail-auction-history-modal-card-content">
      ...
      <div class="detail-auction-history-modal-card">
        ...
        >
          <div class="detail-auction-history-modal-card">
            <div class="detail-auction-history-modal-card-image-container">
              <div class="detail-auction-history-modal-card-image-info">
                <el-image
                  :src="detailAuctionHistoryData.goodsUrl"
                  class="detail-auction-history-modal-card-image"
                  fit="contain"
                />
                <div class="detail-auction-history-modal-card-option-container">
              ...
              <div class="detail-auction-history-modal-card-image-price">
                <div
                  class="detail-auction-history-modal-card-option-gird-price"
                >
                  <p
                    class="detail-auction-history-modal-card-option-gird-title"
                  >
                    정상가
                  </p>
                  <span>
                    {{ getPriceValue(detailAuctionHistoryData.goodsPrice) }}
                  </span>
                </div>
                <div
                  class="detail-auction-history-modal-card-option-gird-price"
                >
                  ...
                  <span>
                    {{ getPriceValue(detailAuctionHistoryData.startPrice) }}
                  </span>
                </div>
                <div
                  class="detail-auction-history-modal-card-option-gird-price"
                >
                  ...
                  <span>
                    {{ getPriceValue(detailAuctionHistoryData.bidPrice) }}
                  </span>
                </div>
              </div>
            </div>
      ...
      <div class="detail-auction-history-modal-card">
        ...
        >
          <div class="detail-auction-history-modal-card">
            <p class="detail-auction-history-modal-card-title">총 입찰 수</p>
            <p class="detail-auction-history-modal-card-content">
              {{ getCountValue(detailAuctionHistoryData.totalBidCount) }}
            </p>
          </div>
        </el-card>
      </div>
      <div class="detail-auction-history-modal-card">
        ...
        >
          <div class="detail-auction-history-modal-card">
            <p class="detail-auction-history-modal-card-title">1등</p>
            <p
              v-if="getBidState(detailAuctionHistoryData, 0)"
              class="detail-auction-history-modal-card-content"
            >
              {{ getTargetBidData(0, 'code') }} -
              {{ getTargetBidData(0, 'price') }}원
            </p>
            <p
              v-else
              class="detail-auction-history-modal-card-content"
            ...
            <p class="detail-auction-history-modal-card-title">2등</p>
            <p
              v-if="getBidState(detailAuctionHistoryData, 1)"
              class="detail-auction-history-modal-card-content"
            >
              {{ getTargetBidData(1, 'code') }} -
              {{ getTargetBidData(1, 'price') }}원
            </p>
            <p
              v-else
              class="detail-auction-history-modal-card-content"
            ...
            <p class="detail-auction-history-modal-card-title">3등</p>
            <p
              v-if="getBidState(detailAuctionHistoryData, 2)"
              class="detail-auction-history-modal-card-content"
            >
              {{ getTargetBidData(2, 'code') }} -
              {{ getTargetBidData(2, 'price') }}원
            </p>
            <p
              v-else
              class="detail-auction-history-modal-card-content"
  ...
  >
    ...
    <div>
      ...
      <el-select
        ...
        class="extra-service-page-select ml-10"
  ...
  <el-table
    v-if="auctionHistoryData.length !== 0"
    :data="auctionHistoryData"
    :height="530"
    border
    class="mb-20"
  >
...
.detail-auction-history-modal {
  display: grid;
  grid-template-columns: 1fr 1.5fr 1fr 1fr;
  gap: 10px;

  .detail-auction-history-modal-card {
    display: flex;
    flex-direction: column;
    gap: 10px;

    .detail-auction-history-modal-card-box {
      height: 100%;
      border-radius: 10px;
      ...
      .detail-auction-history-modal-card {
        ...
        .detail-auction-history-modal-card-title {
          font-size: 19px;
          font-weight: bold;
        }

        .detail-auction-history-modal-card-content {
          margin-left: 10px;
          font-size: 16px;
        }

        .detail-auction-history-modal-card-image-container {
          display: grid;
          grid-template-rows: 2fr 1fr;
          gap: 10px;

          .detail-auction-history-modal-card-image-info {
            display: grid;
            grid-template-columns: 1fr 1fr;

            .detail-auction-history-modal-card-image {
              width: 100%;
              height: 140px;
            }

            .detail-auction-history-modal-card-option-container {
              display: flex;
              flex-direction: column;
          ...
          .detail-auction-history-modal-card-image-price {
            display: grid;
            grid-template-columns: 1fr 1fr 1fr;

            .detail-auction-history-modal-card-option-gird-price {
              display: flex;
              flex-direction: column;
              gap: 20px;
              justify-content: end;
              ...
              .detail-auction-history-modal-card-option-gird-title {
                font-size: 20px;
                font-weight: bold;
              }
            }
