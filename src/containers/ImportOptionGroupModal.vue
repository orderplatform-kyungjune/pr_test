...
import {
  ...
  requestOrderTwoOptionDataType,
  requestOrderTwoOptionMenuType,
...
/** 옵션 데이터 */
const optionList: Ref<requestReferenceOptionDataType[]> = ref(
  [] as requestReferenceOptionDataType[],
);
...
const setOrderOneOptionData = (item: requestReferenceOptionDataType) => {
  const confirmText =
    props.type === 'remove'
      ? '정말 옵션 그룹을 덮어쓰시겠습니까?'
      : '정말 옵션 그룹을 추가하시겠습니까?';
...
const setOrderTwoOptionData = (item: requestReferenceOptionDataType) => {
  const confirmText =
    props.type === 'remove'
...
const onClickAddOrOverwriteOptionGroupButton = (
  optionGroup: requestReferenceOptionDataType,
) => {
...
const getProductListKey = (
  data: requestReferenceOptionDataType,
  index: number,
) => (data ? `${data.goodCode} ${index}` : `product ${index}`);
const getOrderOneOptionGroupListKey = (
  data: requestOrderOneOptionDataType,
  index: number,
) => (data ? `${data.group_num} ${index}` : `product ${index}`);
const getOrderTwoOptionGroupListKey = (
  data: requestOrderTwoOptionDataType,
  index: number,
) => (data ? `${data.option_group_no} ${index}` : `product ${index}`);
const getItemInfoListKey = (
  data: requestOrderOneOptionMenuType | requestOrderTwoOptionMenuType,
  index: number,
) => (data ? `${data.T_order_store_good_code} ${index}` : `product ${index}`);

const isNoneData = computed(() => optionList.value.length === 0);
const isAddType = (type: string) => type === 'add';
const isSoldOut = (
  target: requestOrderOneOptionMenuType | requestOrderTwoOptionMenuType,
) => target.T_order_store_good_option_isSale === 'Y';
const isStopSale = (
  target: requestOrderOneOptionMenuType | requestOrderTwoOptionMenuType,
) => target.T_order_store_good_option_use === 'N';
            ...
            <div class="product-info-card-gird">
              <template v-if="!isStoreApi2">
                <el-card
                  v-for="(
                    group, num
                  ) in product.option as requestOrderOneOptionDataType[]"
                    ...
                    >
                      ...
                      >
                        <div class="group-info-card-code mr-5">
                          {{ info.T_order_store_good_option_code }}
                        </div>
                        <el-row>
                          <el-tag
                      ...
                      <div class="group-info-card-item-grid">
                        <div>
                          {{ info.T_order_store_good_option_name }}
                        </div>
                        <div>
                          {{
                            info.T_order_store_good_option_price?.toLocaleString()
                          }}
                          원
                        </div>
                      </div>
              ...
              <template v-else>
                <el-card
                  v-for="(
                    group, num
                  ) in product.option as requestOrderTwoOptionDataType[]"
