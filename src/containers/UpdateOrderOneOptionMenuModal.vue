...
import { productDetailInfoDataOptionType } from '@interface/goods';
...
const props = defineProps<{
  options: productDetailInfoDataOptionType;
  productInfo: {
    storeCode: string;
    goodCode: string;
    posGoodCode: string;
    ...
    searchedOptionList.value.forEach((initData, i) => {
      props.options.option_item.forEach((optionMenu) => {
        if (
          ...
          initData.O_id === optionMenu.T_order_store_good_option_code
        ) {
          searchedOptionList.value[i].option_qty =
            optionMenu.T_order_store_good_option_select_cnt;
        }
      });
    });
  });
...
onMounted(() => {
  ...
  props.options.option_item.forEach((item: requestOrderOneOptionMenuType) => {
