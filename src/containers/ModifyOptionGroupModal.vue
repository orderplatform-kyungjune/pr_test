...
import {
  modifyOptionGroupsDataType,
  modifyOptionType,
  OptionTypeType,
  optionUpdateListOptionType,
  optionUpdateRequestBodyType,
  optionUpdateRequestDataType,
...
import {
  requestInitGoods,
  requestInitOptions,
  requestOptionGroupUpdate,
  requestSelectedOptionInfo,
} from '@apis/option';
...
const modifyOptionGroupModalData = modalData.modifyOptionGroup as {
  optionGroupNo: number | null;
  onSubmit: () => void;
};

/** form 입력 data */
const requestFormData = reactive<optionUpdateRequestDataType>({
  setting: {
    storeCode: query.code,
    posGoodCode: query.posGoodCode,
    optionGroupNo: modifyOptionGroupModalData.optionGroupNo,
    option_display_name: '',
    option_require: 'Y',
    min_limit_qty: 1,
    max_limit_qty: 0,
...
const listData = reactive<modifyOptionGroupsDataType>({
  originOptions: [],
  originGoods: [],
});
...
const searchOptionCallback = (option: modifyOptionType) =>
...
const isGetSelectedOptionInfoLoading = ref(true);
/** 선택한 옵션의 정보 가져오기 api 요청 및 후처리 */
const getSelectedOptionInfo = async () => {
  try {
    const response = await requestSelectedOptionInfo({
      storeCode: query.code,
      posGoodCode: query.posGoodCode,
      optionGroupNo: modifyOptionGroupModalData.optionGroupNo,
    });

    const { setting, items } = response.data;
    requestFormData.setting = {
      ...requestFormData.setting,
      option_display_name: setting.option_display_name,
      option_require: setting.option_require,
      min_limit_qty: setting.min_limit_qty,
      max_limit_qty: setting.max_limit_qty,
    };

    if (setting.max_limit_qty > 0) {
      isUnlimitedQty.value = false;
    }

    /** 옵션/메뉴 목록 > 이전 데이터 업데이트
     * @param originList - 옵션/메뉴 목록
     * @param selectedOptions - 기존 선택했던 옵션/메뉴 목록
     * @param type - 옵션 or 메뉴
     * @return 기존 선택한 값이 적용된 옵션/메뉴 목록
     */
    const updateOptions = (
      originList: modifyOptionType[],
      selectedOptions: optionUpdateListOptionType[],
      type: OptionTypeType,
    ): modifyOptionType[] =>
      originList.map((item) => {
        const index = selectedOptions.findIndex(
          (selectedOption) => selectedOption.id === item.O_id,
        );

        if (index === -1) return { ...item, depth: false };

        requestFormData.items[type].push(selectedOptions[index].id);
        return {
          ...item,
          option_qty: selectedOptions[index].quantity,
          preset_yn: selectedOptions[index].preset_yn,
          depth: selectedOptions[index].depth,
          use: selectedOptions[index].use,
          soldOut: selectedOptions[index].soldOut,
        };
      });

    listData.originOptions = updateOptions(
      listData.originOptions,
      items.options,
      'option',
    );
    listData.originGoods = updateOptions(
    ...
    isGetSelectedOptionInfoLoading.value = false;
  }
};
...
const getInitOptions = async () => {
  try {
    ...
    listData.originOptions = response.data.map((responseData) => ({
      ...responseData,
      option_qty: 1,
      preset_yn: 'N',
      use: 'Y',
...
const getInitGoods = async () => {
  try {
    ...
    listData.originGoods = response.data.map((responseData) => ({
      ...responseData,
      option_qty: 1,
      preset_yn: 'N',
      use: 'Y',
...
onMounted(async () => {
  await Promise.allSettled([getInitOptions(), getInitGoods()]).then(
 ...
 */
const getSearchedOptions = (options: modifyOptionType[]) =>
  options.filter(searchOptionCallback);

/** 옵션 "전체 선택" 체크박스 상태 */
const isAllOptionsChecked = computed(
  () =>
    requestFormData.items.option.length > 0 &&
    getSearchedOptions(listData.originOptions).every((searchedOption) =>
...
const onChangeOptionCheckboxAll = (state: boolean) => {
  if (state) {
    requestFormData.items.option = getSearchedOptions(
      listData.originOptions,
    ).map((searchedOption) => searchedOption.O_id);
    return;
  }

  requestFormData.items.option = listData.originOptions
    .filter((originOption) => originOption.depth)
    .map((originOption) => originOption.O_id);
};
...
const isAllGoodsChecked = computed(
  () =>
    requestFormData.items.goods.length > 0 &&
    getSearchedOptions(listData.originGoods).every((searchedGoods) =>
...
const onChangeGoodsCheckboxAll = (state: boolean) => {
  if (state) {
    requestFormData.items.goods = getSearchedOptions(listData.originGoods).map(
      (searchedGoods) => searchedGoods.O_id,
    );
    return;
  }

  requestFormData.items.goods = listData.originGoods
    .filter((goods) => goods.depth)
    .map((goods) => goods.O_id);
};

/**
 * 옵션그룹 "수정"시 validation 조건
 * @param selectedOptions - 선택된 옵션/메뉴 목록
 * @return state - validation 검증 결과(모두 통과시 true)
 */
const validateUpdateOptionGroup = (selectedOptions: modifyOptionType[]) => {
  // 옵션그룹명을 입력하지 않았을경우
...
const isModifyOptionGroupLoading: Ref<boolean> = ref(false);
/** 옵션그룹 수정 api 요청 및 후처리 */
const updateOptionGroupUpdate = async () => {
  try {
    ...
    if (!validateUpdateOptionGroup([...resultGoods, ...resultOptions])) {
      return;
    }

    isModifyOptionGroupLoading.value = true;
    const requestBody: optionUpdateRequestBodyType = {
      setting: {
        ...requestFormData.setting,
    ...
    await requestOptionGroupUpdate(requestBody);
    modifyOptionGroupModalData.onSubmit();
    closeModal(MODIFY_OPTION_GROUP);
    ElMessage({
      type: 'success',
      message:
        '정상적으로 수정이 완료되었습니다. 저장하기 위해 변경사항 저장 버튼을 눌러주세요.',
    });
  } catch (error) {
    apiErrorDialogHandler({ error });
  } finally {
    isModifyOptionGroupLoading.value = false;
  }
};
</script>
...
<template>
  <el-dialog
    v-model="flag.modifyOptionGroup"
    :close-on-click-modal="false"
    width="80%"
    align-center
  >
    <template #header>
      <span class="mr-10"> 옵션 그룹 수정 </span>
    </template>
    <div v-loading="isGetSelectedOptionInfoLoading">
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    <el-tooltip
                      v-if="originOption.depth"
                      effect="dark"
                      content="연결된 메뉴, 옵션을 삭제하면 활성화 상태로 변경됩니다"
                      placement="top"
                    >
                      <el-checkbox
                        :label="originOption.O_id"
                        :disabled="true"
                      >
                        {{ originOption.O_id }}
                      </el-checkbox>
    ...
    <template #footer>
      <el-button
        type="danger"
        @click="closeModal(MODIFY_OPTION_GROUP)"
      >
        취소
      </el-button>

      <el-button
        v-loading="isModifyOptionGroupLoading"
        class="submit-loading-button"
        type="primary"
        :disabled="!isOptionSelected"
        @click="updateOptionGroupUpdate"
      >
        수정
      </el-button>
    </template>
