...
/** 티오더2 옵션 상품 리스트 */
const optionMenuList: Ref<orderTwoOptionMenuDataType[]> = ref([]);

/** 티오더2 옵션 그룹 정보 (현 order2에서는 is_type 값이 G든 I든 상관없는 값이 됨.) */
const optionGroupData: Ref<orderTwoOptionGroupDataType> = ref({
  is_type: 'G',
} as orderTwoOptionGroupDataType);
...
const setCheckedItemSelectCount = () => {
  selectedOptionList.value.forEach((selectedOptionCode) => {
    const targetInfo = optionGroupData.value.item.find(
      (checked) =>
        checked.T_order_store_good_option_code === selectedOptionCode,
    );
    ...
    if (targetInfo && targetIndex !== -1) {
      searchedOptionList.value[targetIndex].option_qty =
        targetInfo.T_order_store_good_option_select_cnt;
    }
  });
...
watch(optionGroupData, () => {
  ...
  optionGroupData.value.item?.forEach((item: orderTwoOptionMenuDataType) => {
    checkedItem?.push(item.T_order_store_good_option_code);
  });
...
// v-for 방어 코드
const getOptionMenuInPosKey = (item: requestPosInitDataType, index: number) =>
      ...
      <div class="option-list-container">
        ...
        >
          <div
            ...
            :key="getOptionMenuInPosKey(posItem, index)"
            class="item-box"
          >
            <el-checkbox-group
