...
import { RefreshTabletModal } from '@containers';
...
import { helper, store, tablet } from '@apis';
...
watch(
  () => storeDetailData.value.platform_store_Id,
  () => {
    activeDuplicateCheck.tabletId = !!(
      storeDetailData.value.platform_store_Id &&
      storeDetailData.value.platform_store_Id !== savedData.tabletId
    );
    isPassedDuplicateData.tabletId = storeDetailData.value.platform_store_Id
      ? !activeDuplicateCheck.tabletId
      : true;
  },
);
...
/** 매장 배너 데이터 */
const storeBannerData: Ref<storeBannerInfoPlatformType[]> = ref(
  [] as storeBannerInfoPlatformType[],
);

// 광고 배너 리스트
const storeAdBannerList: Ref<storeAdBannerListPlatformType[]> = ref(
  [] as storeAdBannerListPlatformType[],
);
...
const addBasicLanguageInList = () => {
  if (
    storeDetailData.value.platform_store_language === 'CN' &&
    !storeDetailData.value.platform_store_use_language_list?.includes('zh_hant')
  ) {
    storeDetailData.value.platform_store_use_language_list?.push('zh_hant');
    return;
  }
  if (
    storeDetailData.value.platform_store_language === 'CN-S' &&
    !storeDetailData.value.platform_store_use_language_list?.includes('zh_hans')
  ) {
    storeDetailData.value.platform_store_use_language_list?.push('zh_hans');
    return;
  }
  if (
    storeDetailData.value.platform_store_language === 'CN' ||
  ...
  const lowerLang =
    storeDetailData.value.platform_store_language?.toLocaleLowerCase();
  if (
    !storeDetailData.value.platform_store_use_language_list?.includes(lowerLang)
  ) {
    storeDetailData.value.platform_store_use_language_list?.push(lowerLang);
  }
};
  ...
  try {
    ...
    if (!typeError) {
      if (res.data.code === 200) {
        ...
        storeDetailData.value.platform_store_Ad_banner_list =
          res.data.data.store.platform_store_Ad_banner_list;
        storeDetailData.value.platform_store_use_language_list =
          res.data.data.store.platform_store_use_language_list;
        ...
        savedData.tabletId = res.data.data.store?.platform_store_Id || '';
...
const setLogoImage = async () => {
  ...
  try {
    ...
    const data = {
      storeCode,
      platform_logo_img: logoImage,
    };

    const res = (await requestLogoImageUpdateInPlatform(data)) as AxiosResponse;
...
const updateStoreBgImg = async () => {
  ...
  try {
    ...
    const data: requestUpdateStoreBackgroundImagePlatformType = {
      storeCode,
      platform_store_background: backgroundImg,
    };

    const res = (await requestUpdateStoreBackgroundImageInPlatform(
      data,
    )) as AxiosResponse;
...
const updateRestroomImg = async () => {
  ...
  try {
    ...
    const data: requestUpdateRestroomImagePlatformType = {
      storeCode,
      platform_store_restroom_img: backgroundImg,
    };

    const res = (await requestUpdateRestroomImageInPlatform(
      data,
    )) as AxiosResponse;
...
const postStoreUpdate = async () => {
  ...
  const requestStoreData = JSON.parse(JSON.stringify(storeDetailData.value));

  const storeTabletOrder =
    requestStoreData.platform_store_Tablet_sort as storeTabletSortType;
  const editPropertyColumn = {
    platform_store_Tablet_sort: storeTabletOrder?.column as string,
  };
  const editPropertyOrder = {
    platform_store_orderBy: storeTabletOrder?.order as string,
  };
  ...
  delete requestStoreData.platform_store_Tablet_sort;
  ...
  storeDetailData.value.platform_store_Ad_banner_list?.sort((a, b) => a - b);

  // uplus: 인원수 그룹명 기능 사용을 위해 인원 입력 기능 고정
  requestStoreData.visitGroupUse = 'Y';
    ...
    ElMessageBox.confirm('매장 정보를 수정하시겠습니까?', '경고', {
      ...
      try {
        ...
        if (res.data.code === 200) {
          ...
          if (
            beforeModificationStoreName.value !==
            storeDetailData.value.platform_store_name
          ) {
            saveScrollPosition();
            await router.replace({
              query: {
                code: route.query.code as string,
                name: storeDetailData.value.platform_store_name,
              },
            });
...
const validateRequiredData = async () => {
  ...
  if (!storeDetailData.value.platform_store_staff_password) {
    ElMessage({
      type: 'error',
  ...
  if (!storeDetailData.value.platform_store_Id) {
    ElMessage({
      type: 'error',
  ...
  if (!storeDetailData.value.platform_store_name) {
    ElMessage({
      type: 'error',
  ...
  if (!storeDetailData.value.platform_store_Theme) {
    ElMessage({
      type: 'error',
      message: '테마를 선택해주세요.',
    });
...
const storeLogoUploadSuccess: UploadProps['onSuccess'] = (response) => {
  storeDetailData.value.platform_store_logo = URL.createObjectURL(response.raw);
...
const storeBgImgUploadSuccess: UploadProps['onSuccess'] = (response) => {
  storeDetailData.value.platform_store_background = URL.createObjectURL(
    response.raw,
  );
...
const storeRestroomUploadSuccess: UploadProps['onSuccess'] = (response) => {
  storeDetailData.value.platform_store_restroom_img = URL.createObjectURL(
    response.raw,
  );
...
const postCheckDuplicateId = async () => {
  ...
  try {
    const adminIdInfo = {
      storeCode,
      platform_id: storeDetailData.value.platform_store_Id,
    };

    if (adminIdInfo.platform_id === '') {
      ElMessage({
        type: 'error',
        message: '아이디를 입력해주세요.',
      });
    ...
    if (res.data.code === 200) {
      if (!res.data.result) {
        ...
        ElMessage({
          type: 'error',
          message:
            '사용할 수 없는 태블릿 로그인 아이디 입니다. 다시 시도해 주세요.',
        });
      }
    }
  } catch (error) {
...
const postCheckDuplicateMiddleWareCode = async () => {
  ...
  try {
    ...
    if (res.data.code === 200) {
      ...
      ElMessage({
        type: 'error',
        message: '사용할 수 없는 미들웨어 아이디 입니다. 다시 시도해 주세요.',
      });
    }
  } catch (error) {
    console.log(error);
...
// 선택된 스토어 정보
const selectedStoreInfo = reactive<storeInfoOfStoreManagePlatformType>({
  storeCodeArray: [],
  storeBriefInfo: [],
});
...
const openRefreshTabletModal = () => {
  ...
  selectedStoreInfo.storeBriefInfo = [
    {
      platform_store_name: storeName,
      platform_store_code: storeCode,
    },
  ];
...
const isValidAddress = (value: string) => {
  if (storeDetailData.value.platform_store_address) {
    storeDetailData.value.platform_store_drive_call_hide = value;
    return;
  }
  ...
  storeDetailData.value.platform_store_drive_call_hide = 'N';
};
  ...
  const daumPostSearch = () => {
    window.daum.postcode.load(() => {
      const searchPost = new window.daum.Postcode({
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        oncomplete: (data: any) => {
          if (storeDetailData.value.platform_store_address2 !== '') {
            storeDetailData.value.platform_store_address2 = '';
          }

          if (data.userSelectedType === 'R') {
            // 사용자가 도로명 주소를 선택했을 경우
            storeDetailData.value.platform_store_address = data.roadAddress;
          } else {
            // 사용자가 지번 주소를 선택했을 경우(J)
            storeDetailData.value.platform_store_address = data.jibunAddress;
          }
          ...
          if (data.userSelectedType === 'R') {
            ...
            if (data.bname !== '' && /[동|로|가]$/g.test(data.bname)) {
              storeDetailData.value.platform_store_address2 += data.bname;
            }
            ...
            if (storeDetailData.value.platform_store_address2 !== '') {
              storeDetailData.value.platform_store_address2 = `(${storeDetailData.value.platform_store_address2})`;
            }
          } else {
            storeDetailData.value.platform_store_address2 = '';
          }
          // 우편번호를 입력한다.
          storeDetailData.value.platform_store_zipcode = data.zonecode;
        },
      });
...
// 미들웨어 Y인 경우: POS 항상 O
watch(
  () => storeDetailData.value.platform_store_has_middleware,
  () => {
    if (storeDetailData.value.platform_store_has_middleware === 'Y') {
      storeDetailData.value.platform_store_has_pos = 'Y';
    }
  },
);

watch(
  () => storeDetailData.value.platform_store_has_pos,
  (newValue, oldValue) => {
    if (
      storeDetailData.value.platform_store_has_middleware === 'N' &&
      storeDetailData.value.platform_store_has_pos === 'N'
    ) {
      storeDetailData.value.platform_store_pos_info = '노포스(자체포스)';
      isOnlyNoPos.value = true;
      return;
    }

    if (newValue === 'Y' && oldValue === 'N') {
      storeDetailData.value.platform_store_pos_info = '';
...
<template>
  <RefreshTabletModal
    ...
    :storeCode="selectedStoreInfo.storeBriefInfo[0]?.platform_store_code"
    :storeName="selectedStoreInfo.storeBriefInfo[0]?.platform_store_name"
  />
  ...
  <div class="store-manage-contents">
    ...
    >
      ...
      >
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_Id)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>
          ...
          <div class="store-manage-contents-duplicate-check">
            <div class="store-manage-contents-duplicate-input">
              <el-input
                ...
                v-model="storeDetailData.platform_store_Id"
                clearable
        ...
        <div class="store-manage-contents-contents-wrapper">
          ...
          <form>
            <el-input
              v-model="storeDetailData.platform_store_pwd"
        ...
        <div class="store-manage-contents-contents-wrapper">
          ...
          <form>
            <el-input
              v-model="storeDetailData.platform_store_pwd_re"
              autocomplete="new-password"
              clearable
              placeholder="Password"
              show-password
        ...
        <div class="store-manage-contents-contents-wrapper">
          ...
          <el-input
            ...
            v-model="storeDetailData.platform_store_staff_password"
            clearable
            oninput="this.value = this.value.replace(/\D/g, '').replace(/(\..*)\./g, '$1');"
          />
        </div>
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_direct_table)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
          ...
          <el-radio-group v-model="storeDetailData.platform_store_direct_table">
            <el-radio
              border
              label="Y"
            >
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_ipv4)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>태블릿 접근 IP</div>
          ...
          <el-input
            v-model="storeDetailData.platform_store_ipv4"
            clearable
          />
        </div>
        <div class="tablet-control-wrap mb-20">
          ...
          <div
            v-if="
              isUndefinedData(storeDetailData.platform_store_tablet_version)
            "
            class="store-manage-contents-contents-wrapper"
          >
            ...
            <div class="store-manage-url-contents font-thin">
              <el-text class="ml-10">
                {{ storeDetailData.platform_store_tablet_version }}
              </el-text>
            </div>
          </div>
          <div
            v-if="isUndefinedData(storeDetailData.platform_store_Theme)"
            class="store-manage-contents-contents-wrapper"
          >
            <div class="store-manage-contents-contents-title">
              <div>
          ...
          <div
            v-if="
              isUndefinedData(storeDetailData.platform_store_orderView_version)
            "
            class="store-manage-contents-contents-wrapper"
          >
            <div class="store-manage-contents-contents-title">
            ...
            <div class="store-manage-url-contents font-thin">
              <el-text class="ml-10">
                {{ storeDetailData.platform_store_orderView_version }}
              </el-text>
            </div>
          </div>
        </div>
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_Tablet_sort)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
          ...
          >
            <el-radio-group
              v-model="storeDetailData.platform_store_Tablet_sort.column"
              class="mb-10"
            >
              <el-radio
                border
          ...
          >
            <el-radio-group
              v-model="storeDetailData.platform_store_Tablet_sort.order"
            >
              <el-radio
                border
                label="ASC"
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_service_use)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>서비스 요청 설정</div>
          ...
          <el-radio-group v-model="storeDetailData.platform_store_service_use">
            <el-radio
              :label="0"
              border
            >
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_banner_text)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>배너 TEXT 입력</div>
          ...
          <el-input
            v-model="storeDetailData.platform_store_banner_text"
            clearable
          />
        </div>
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_Main_rows)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
          ...
          <el-radio-group v-model="storeDetailData.platform_store_Main_rows">
            <el-radio
              border
              label="2Pcs"
            >
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_banner_Ad_use)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>배너 사용</div>
          ...
          <el-radio-group
            v-model="storeDetailData.platform_store_banner_Ad_use"
          >
            <el-radio
              border
              label="Y"
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_banner_time)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>배너/로고 노출 시간</div>
          ...
          <el-input-number
            v-model="storeDetailData.platform_store_banner_time"
            :max="300"
            :min="10"
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_banner_control)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>배너 컨트롤</div>
          ...
          <el-radio-group
            v-model="storeDetailData.platform_store_banner_control"
          >
            <el-radio
              border
              label="Y"
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_guide_use)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
          ...
          <el-radio-group v-model="storeDetailData.platform_store_guide_use">
            <el-radio
              border
              label="Y"
            >
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_categoryListUp)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
          ...
          <el-radio-group
            v-model="storeDetailData.platform_store_categoryListUp"
          >
            <el-radio
              :label="1"
              border
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_cartPage_override)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>장바구니 덮어보이기</div>
          ...
          <el-radio-group v-model="storeDetailData.platform_cartPage_override">
            <el-radio
              :label="1"
              border
            >
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_simpleView)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>간단히 보기</div>
          ...
          <el-radio-group v-model="storeDetailData.platform_simpleView">
            <el-radio
              border
              label="Y"
            >
    ...
    <div
      ...
      class="store-manage-contents-container"
    >
      <div class="store-manage-contents-title">기본 언어 설정</div>
      <div
        ref="torderMasterLanguage"
        class="store-manage-contents-contents"
      >
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_language)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
          ...
          <el-select
            v-model="storeDetailData.platform_store_language"
            class="width-100"
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_language_use)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>언어 사용</div>
          ...
          <el-radio-group v-model="storeDetailData.platform_store_language_use">
            <el-radio
              border
              label="Y"
            >
        ...
        <div
          v-if="
            isUndefinedData(storeDetailData.platform_store_use_language_list)
          "
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
          ...
          <el-checkbox-group
            v-model="storeDetailData.platform_store_use_language_list"
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_master_language)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
          ...
          <el-select
            v-model="storeDetailData.platform_store_master_language"
            class="width-100"
          >
            <el-option
              v-for="lang in basicLanguageList"
    ...
    >
      ...
      >
        <div
          v-if="
            isUndefinedData(storeDetailData.platform_store_contract_startDate)
          "
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
          ...
          <el-date-picker
            v-model="storeDetailData.platform_store_contract_startDate"
            class="ml-10"
            type="Date"
          ...
          <el-date-picker
            v-model="storeDetailData.platform_store_contract_endDate"
            class="ml-10"
            type="Date"
            value-format="YYYY-MM-DD"
          />
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_upjong)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>매장 업종 설정</div>
          ...
          <el-input
            v-model="storeDetailData.platform_store_upjong"
            clearable
          />
        </div>
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_upte)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>매장 업태 설정</div>
          ...
          <el-input
            v-model="storeDetailData.platform_store_upte"
            clearable
          />
        </div>
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_contractNumber)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>계약자 번호 설정</div>
          ...
          <el-input
            v-model="storeDetailData.platform_store_contractNumber"
            clearable
          />
        </div>
        <div
          v-if="
            isUndefinedData(storeDetailData.platform_store_tablet_installer)
          "
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
          ...
          <el-input
            v-model="storeDetailData.platform_store_tablet_installer"
            clearable
          />
        </div>
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_ordertablet_cnt)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
          ...
          <el-input-number
            v-model="storeDetailData.platform_store_ordertablet_cnt"
            :min="0"
          />
        </div>
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_viewtablet_cnt)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
          ...
          <el-input-number
            v-model="storeDetailData.platform_store_viewtablet_cnt"
            :min="0"
          />
        </div>
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_extratablet_cnt)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
          ...
          <el-input-number
            v-model="storeDetailData.platform_store_extratablet_cnt"
            :min="0"
          />
        </div>
      </div>
    ...
    >
      ...
      >
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_Ad_use)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>광고 배너 사용</div>
          ...
          <el-radio-group v-model="storeDetailData.platform_store_Ad_use">
            <el-radio
              :label="'Y'"
              border
            >
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_Ad_banner_list)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
          ...
          <el-checkbox-group
            v-model="storeDetailData.platform_store_Ad_banner_list"
            class="ad-banner-wrapper"
          >
            <div
              v-for="(bannerInfo, index) in storeAdBannerList"
              :key="
                bannerKeyValidate(bannerInfo.platform_store_banner_name, index)
              "
            >
              <el-image
                :src="bannerInfo.platform_store_banner_url"
              ...
              <div class="ad-banner-info mt-10">
                <el-tag
                  ...
                  type="info"
                >
                  {{ bannerInfo.platform_store_banner_name }}
                </el-tag>
                <el-checkbox
                  :label="bannerInfo.id"
                  border
    ...
    >
      ...
      >
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_checkout_Ad_use)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>주문 완료 광고 표기</div>
          ...
          <el-radio-group
            v-model="storeDetailData.platform_store_checkout_Ad_use"
          >
            <el-radio
              border
              label="Y"
    ...
    >
      ...
      >
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_close)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>태블릿 점검 중 표시</div>
          ...
          <el-radio-group v-model="storeDetailData.platform_store_close">
            <el-radio
              :label="1"
              border
            >
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_close_order)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>장바구니 주문 가능 여부</div>
          </div>
          <el-radio-group v-model="storeDetailData.platform_store_close_order">
            <el-radio
              border
              label="Y"
            >
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_recent_order_hide)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>주문 내역</div>
          ...
          <el-radio-group v-model="storeDetailData.platform_recent_order_hide">
            <el-radio
              border
              label="Y"
            >
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_order_confirm)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>주문 시 확인 창</div>
          ...
          <el-radio-group
            v-model="storeDetailData.platform_store_order_confirm"
          >
            <el-radio
              border
              label="Y"
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_set_use)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>세트 상품 사용</div>
          </div>
          <el-radio-group v-model="storeDetailData.platform_store_set_use">
            <el-radio
              border
              label="Y"
            >
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_van_info)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>Van사 정보</div>
          ...
          <el-select
            v-model="storeDetailData.platform_store_van_info"
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_visitBook)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>1. 인원 수 선택</div>
          ...
          <el-radio-group v-model="storeDetailData.platform_store_visitBook">
            <el-radio
              border
              label="Y"
            >
        ...
        >
          <div class="store-manage-contents-contents-title">
            <div>
              3. 인원 수 그룹명 등록 (ex: 남자;여자 OR 내국인;외국인) 최대 2개
              가능
            </div>
            <span>
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_drive_call_hide)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
          ...
          <el-radio-group
            v-model="storeDetailData.platform_store_drive_call_hide"
    ...
    >
      ...
      <div
        ...
        class="store-manage-contents-contents"
      >
        <!-- platform_store_pos_info가 없는 경우 미들웨어, 포스 정보도 불필요 -->
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_pos_info)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>미들웨어 연동 여부</div>
          ...
          <el-radio-group
            v-model="storeDetailData.platform_store_has_middleware"
          >
            <el-radio
        ...
        >
          ...
          <el-radio-group
            ...
            v-model="storeDetailData.platform_store_has_pos"
          >
            <el-radio
              id="store-manage-contents-has-pos"
              border
            ...
            <el-radio
              :disabled="storeDetailData.platform_store_has_middleware === 'Y'"
              border
              class="ml-10"
              label="N"
            >
        ...
        >
          ...
          <el-select
            ...
            v-model="storeDetailData.platform_store_pos_info"
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_origin_store_name)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>매장명</div>
          ...
          <el-input
            v-model="storeDetailData.platform_origin_store_name"
            :disabled="!checkAuthFunction('F3003')"
          />
        </div>
        <div
          v-if="isUndefinedData(storeDetailData.platform_area_name)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>지점명</div>
          ...
          <el-input
            v-model="storeDetailData.platform_area_name"
            :disabled="!checkAuthFunction('F3003')"
          />
        </div>
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_name)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>
          ...
          <el-input
            ...
            v-model="storeDetailData.platform_store_name"
            clearable
          />
        </div>
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_Saup_number)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>사업자 번호</div>
          ...
          <el-input
            v-model="storeDetailData.platform_store_Saup_number"
            clearable
          />
        </div>

        <div
          v-if="isUndefinedData(storeDetailData.platform_store_address)"
          class="store-manage-contents-contents-wrapper"
        >
          ...
          >
            ...
            <el-input
              id="postCode"
              v-model="storeDetailData.platform_store_zipcode"
              class="address-input"
              clearable
            />
          </el-row>
          ...
          >
            ...
            <el-input
              id="address"
              v-model="storeDetailData.platform_store_address"
              class="address-input"
              clearable
            />
          </el-row>
          ...
          >
            ...
            <el-input
              id="detailAddress"
              v-model="storeDetailData.platform_store_address2"
              class="address-input"
              clearable
            />
          </el-row>
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_Popup_time)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>리뷰 서비스 시간</div>
          ...
          <el-input-number
            v-model="storeDetailData.platform_store_Popup_time"
            :min="0"
          />
        </div>
        <div
          v-if="isUndefinedData(storeDetailData.platform_stote_register_use)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>가입하기 사용</div>
          ...
          <el-radio-group v-model="storeDetailData.platform_stote_register_use">
            <el-radio
              border
              label="Y"
            >
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_point_use)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>포인트 사용</div>
          ...
          <el-radio-group v-model="storeDetailData.platform_store_point_use">
            <el-radio
              border
              label="Y"
            >
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_cart_point_type)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>결제시 포인트</div>
          ...
          <div class="mb-10">
            <el-radio-group
              v-model="storeDetailData.platform_store_cart_point_type"
            >
              <el-radio
                :label="0"
                border
          ...
          <el-input
            v-model="storeDetailData.platform_store_cart_point"
            clearable
          />
        </div>
        <div
          v-if="isUndefinedData(storeDetailData.platform_min_order_price)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>최소 주문 금액 설정</div>
          ...
          <el-input
            v-model="storeDetailData.platform_min_order_price"
            clearable
          />
        </div>
        <div
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_restroom_use)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
          ...
          <el-radio-group v-model="storeDetailData.platform_store_restroom_use">
            <el-radio
              border
              label="Y"
            >
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_restroom_img)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>화장실 안내 이미지</div>
          ...
          >
            ...
            >
              <div class="mb-10">
                <el-image
                  :src="storeDetailData.platform_store_restroom_img"
                  alt="upload-image"
                  class="upload-image"
                >
                  <template #error>
    ...
    >
      ...
      >
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_logo)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>매장 로고</div>
          ...
          >
            ...
            >
              <div class="mb-10">
                <el-image
                  :src="storeDetailData.platform_store_logo"
                  alt="upload-image"
                  class="upload-image"
                >
                  <template #error>
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_logoUse)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>로고 사용</div>
          ...
          <el-radio-group v-model="storeDetailData.platform_store_logoUse">
            <el-radio
              border
              label="Y"
            >
    ...
    >
      ...
      >
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_background)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>매장 배경</div>
          ...
          >
            ...
            >
              <div class="mb-10">
                <el-image
                  :src="storeDetailData.platform_store_background"
                  alt="upload-image"
                  class="upload-image"
                >
                  <template #error>
        ...
        <div
          v-if="isUndefinedData(storeDetailData.platform_store_backgroundUse)"
          class="store-manage-contents-contents-wrapper"
        >
          <div class="store-manage-contents-contents-title">
            <div>배경 사용</div>
          ...
          <el-radio-group
            v-model="storeDetailData.platform_store_backgroundUse"
          >
            <el-radio
              border
              label="Y"
