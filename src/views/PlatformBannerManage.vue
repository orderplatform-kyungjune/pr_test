...
import { storeBannerListPlatformType } from '@interface/banner';
import { Picture } from '@element-plus/icons-vue';
import { PlatformAddBannerModal } from '@containers';
...
/** 매장 배너 리스트 정보 */
const storeBannerList: Ref<storeBannerListPlatformType[]> = ref(
  [] as storeBannerListPlatformType[],
);
...
/** 배너 등록하기 */
const updateBannerInfo = (target: storeBannerListPlatformType) => {
  const bannerData = {
    storeCode: route.query.code as string,
    id: target.id,
    platform_store_banner_use: target.platform_store_banner_use,
    platform_store_banner_sort: target.platform_store_banner_sort,
    platform_store_banner_time: target.platform_store_banner_time,
  };
  ...
  ElMessageBox.confirm('배너를 수정 하시겠습니까?', '배너 수정', {
    ...
    .then(async () => {
      try {
        const res = (await requestUpdateBannerInPlatform(
          bannerData,
        )) as AxiosResponse;
...
/** 배너 삭제하기 */
const postDeleteBanner = async (target: storeBannerListPlatformType) => {
  const bannerData = {
    storeCode: route.query.code as string,
    id: target.id,
  };
...
// 삭제하기
const setDeleteCheck = (target: storeBannerListPlatformType) => {
  if (!target) return;
  const categoryName = target.platform_store_banner_name;
  ...
  >
    ...
    >
      <template #default="scope">
        <el-radio-group v-model="scope.row.platform_store_banner_use">
          <el-radio
            border
            label="Y"
          >
    ...
    >
      <template #default="scope">
        <el-select v-model="scope.row.platform_store_banner_sort">
          <el-option
    ...
    >
      <template #default="scope">
        {{ scope.row.platform_store_banner_name }}
      </template>
    </el-table-column>
    <el-table-column
      align="center"
    ...
    >
      <template #default="scope">
        <el-image
          :src="scope.row.platform_store_banner_url"
    ...
    >
      <template #default="scope">
        <el-input-number
          v-model="scope.row.platform_store_banner_time"
          :max="30"
          :min="5"
          :step="5"
        />
