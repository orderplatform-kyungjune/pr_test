...
import { authentication, runtimeCheckHelper } from '@utils/index';
...
import { SetTabletVersionModal } from '@containers';
import {
  BreadcrumbHeader,
  ExampleBackground,
  ExampleButton,
  ExampleMainCategoryMenu,
  ExampleStaffCallButton,
...
const { storeThemeListCodec, storeDefaultLayoutCodec, storeInfoCodec } =
  storeCodec;
...
const storeDetailData: Ref<Partial<requestStoreInfoDataStoreType>> = ref({
  T_order_store_Theme: '',
  optionLayout: '',
  T_order_store_staff_password: '',
});

/** 매장 설정 데이터 */
const originStoreData = ref({} as storeInfoDataStoreType);
...
const isCustomTheme = () =>
  storeDetailData.value.T_order_store_Theme?.includes(CUSTOM_THEME_CODE);
...
const postCustomStyleInfo = async () => {
  try {
    ...
    const customStyleInfo = {
      ...
      T_order_store_tablet_custom_style: isCustomTheme()
        ? customStyleData.value
        : unfilledCustomStyle,
    };
    const res = (await requestCustomStyleData(
      customStyleInfo,
    )) as AxiosResponse;
...
const postCustomBackgroundImage = async () => {
  ...
  try {
    ...
    const res = (await requestUpdateCategoryBackGroundImage(
      backgroundImageData,
    )) as AxiosResponse;
    ...
    if (res.data.code === 200) {
      customStyleData.value.category.backgroundImg =
        res.data.data.T_order_store_category_background;
    }
  } catch (error) {
    console.log(error);
  }
...
const setDefaultCustomStyle = () => {
  ...
  const hasCustomizedStyle =
    ...
    originStoreData.value.T_order_store_tablet_custom_style.useYn === 'Y';
  // eslint-disable-next-line vue/max-len
  const isValidCustomStyle =
    !!originStoreData.value.T_order_store_tablet_custom_style?.background
      .backgroundColor &&
    !!originStoreData.value.T_order_store_tablet_custom_style?.button
      .backgroundColor;
...
const setCustomStylePart = () => {
  if (storeDetailData.value.T_order_store_Theme === CUSTOM_THEME_CODE) {
  ...
  try {
    ...
    if (!typeError) {
      if (res.data.code === 200) {
        ...
        storeDetailData.value.T_order_store_Theme =
          res.data.data.store.T_order_store_Theme;
        storeDetailData.value.T_order_store_staff_password =
          res.data.data.store.T_order_store_staff_password;
...
const editStoreInfo = async () => {
  ...
  const requestEditData: requestStoreInfoDataStoreType = Object.assign(
    requestStoreData,
    storeCode,
  );
...
const initTabletDisplayData = async () => {
  ...
  customStyleData.value.background.backgroundColor = originStoreData.value
    .T_order_store_tablet_custom_style?.background?.backgroundColor as string;
};
  ...
  <div class="custom-theme-manage-container">
    ...
    <div class="tabs-contents-wrapper">
      ...
      <el-collapse v-model="activeToggleName">
        ...
        >
          <div>
            <p class="color-picker-title mb-10">
              *1/0/4 버전부터 직원 호출 버튼의 커스텀은 불가능합니다.
            </p>
            <div class="color-picker-grid">
              <ExampleStaffCallButton
                :staffCall="customStyleData.staffCallButton"
              />
              <div class="color-picker-setting-grid">
                <el-row align="middle">
                  <p class="color-picker-title mr-10">배경 색상</p>
                  <el-color-picker
                    v-model="customStyleData.staffCallButton.backgroundColor"
                    @change="changeTemplateSettings"
                  />
                </el-row>
                <el-row align="middle">
                  <p class="color-picker-title mr-10">폰트 색상</p>
                  <el-color-picker
                    v-model="customStyleData.staffCallButton.fontColor"
                    @change="changeTemplateSettings"
                  />
                </el-row>
              </div>
