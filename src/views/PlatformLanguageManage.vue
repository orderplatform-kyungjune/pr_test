...
import {
  optionGroupListPlatformType,
  optionGroupPlatformType,
...
import {
  orderTwoOptionGroupListPlatformType,
  orderTwoOptionMenuListPlatformType,
...
const { failAuthenticationAlert, isGlobalAdmin, checkAuthFunction } =
  authentication;
...
const optionGroupList: Ref<optionGroupListPlatformType[]> = ref([]);
const orderTwoOptionGroupList: Ref<orderTwoOptionGroupListPlatformType[]> = ref(
  [],
);
const orderTwoOptionMenuList: Ref<orderTwoOptionMenuListPlatformType[]> = ref(
  [],
);
const selectedDetailProductData: Ref<productDetailInfoDataPlatformType> = ref(
  {} as productDetailInfoDataPlatformType,
);
const selectedOptionGroup: Ref<optionGroupListPlatformType> = ref(
  {} as optionGroupListPlatformType,
);
const selectedOptionGroupTranslationInfo =
  reactive<requestTranslateOptionGroupPlatformType>(
    {} as requestTranslateOptionGroupPlatformType,
  );
...
const disableDetailManualTranslate = (
  detailItem: requestTranslateProductDetailPlatformType,
) => {
...
const getTranslateProductDetailParams = () => {
  ...
  languageArray.forEach((item) => {
    requestTranslateProductDetailParams.value.push({
      ...
      storeCode: pathQuery.code as string,
      platform_store_good_code: selectedDetailProductData.value.goodCode,
...
const requestTranslateProductDetails = async (
  params: requestTranslateProductDetailPlatformType,
  langType?: string,
) => {
  ...
  ElMessageBox.confirm(
    ...
    try {
      loading.value = true;
      const res = (await requestTranslateProductDetailInPlatform(
        params,
      )) as AxiosResponse;
...
const selectedOptionGroupData: Ref<optionGroupPlatformType> = ref(
  {} as optionGroupPlatformType,
);

// 선택 옵션그룹 정보 가져오기
const getSelectedOptionGroupTranslationInfo = (
  optionGroup: optionGroupPlatformType,
) => {
  if (optionGroup) {
    selectedOptionGroupTranslationInfo.platform_good_option_no =
...
const getSelectedOptionGroup = (goodsCode: string) => {
  const filteredItem = optionGroupList.value
    .filter((item) => item.platform_store_good_code === goodsCode)
    .pop();
  if (filteredItem) {
    // 다른 상품 선택 시 tab index 0으로 초기화
    if (selectedOptionGroup.value.platform_store_good_code) {
      if (
        selectedOptionGroup.value.platform_store_good_code !==
        filteredItem.platform_store_good_code
      ) {
...
const selectedTableOptionMenuList: Ref<
  requestOrderOneOptionMenuPlatformType[]
> = ref([]);
const clickGetOptionGroupData = (
  goodCode: string,
  groupData: optionGroupPlatformType,
) => {
...
const refreshOptionMenuLanguageData = (langGoodCode: string) => {
  const targetOptionGroupInList = optionGroupList.value.find(
    (oGroupInList: optionGroupListPlatformType) =>
      oGroupInList.platform_store_good_code === langGoodCode,
  );
  const targetOptionGroup = targetOptionGroupInList?.option_group.find(
    (oGroup: optionGroupPlatformType) =>
...
// 옵션 단일 번역 수정 모달
const openPlatformOptionTranslateModal = (
  type: string,
  goodCode: string,
  optionData: any,
) => {
  const data = {
    ...
    platform_good_option_no: optionData.platform_good_option_no, // api1 옵션그룹,옵션 상품 모두 사용: 옵션 그룹의 group_num
    platform_good_option_group_no: optionData.platform_good_option_group_no, // api2 옵션그룹: 옵션그룹 넘버
    platform_store_good_option_code: optionData.platform_store_good_option_code, // api2 옵션상품: 옵션그룹 코드
    ko: optionData.ko,
    en: optionData.en,
    jp: optionData.jp,
...
// 티오더1 옵션그룹 단일번역 수정
const optionGroupOneUpdate = (
  goodCode: string,
  groupData: optionGroupPlatformType,
) => {
  clickGetOptionGroupData(goodCode, groupData);
  openPlatformOptionTranslateModal('option_group', goodCode, {
    origin_name: groupData.name,
    platform_good_option_no: groupData.group_num,
  ...
  >
    ...
    >
      ...
      >
        ...
        <el-row align="middle">
          <el-button
            v-if="checkAuthFunction('F2001') && !isGlobalAdmin()"
            id="F2001"
                ...
                <el-row>
                  <el-col>
                    <el-tag
                      ...
                      type="info"
                    >
                      {{ scope.row.platform_store_good_code }}
                    </el-tag>
                  </el-col>
                  <el-col>
                    <span class="font-small">
                      {{ scope.row.platform_store_good_display_name }}
                    </span>
                  </el-col>
                </el-row>
              </template>
        ...
        >
          ...
          >
                ...
                <el-row>
                  <el-col>
                    <el-tag
                      ...
                      type="info"
                    >
                      {{ scope.row.platform_store_good_option_code }}
                    </el-tag>
                  </el-col>
                  <el-col>
                    <span>
                      {{ scope.row.platform_store_good_option_name }}
                    </span>
                  </el-col>
                </el-row>
              </template>
            ...
            <el-table-column
              ...
              header-align="center"
              label="한글"
              prop="platform_store_good_option_name_array.ko"
            />
            <el-table-column
              align="center"
              header-align="center"
              label="영어"
              prop="platform_store_good_option_name_array.en"
            />
            <el-table-column
              align="center"
              header-align="center"
              label="중국어(간체)"
              prop="platform_store_good_option_name_array.zh_hans"
            />
            <el-table-column
              align="center"
              header-align="center"
              label="중국어(번체)"
              prop="platform_store_good_option_name_array.zh_hant"
            />
            <el-table-column
              align="center"
              header-align="center"
              label="일본어"
              prop="platform_store_good_option_name_array.jp"
            />
            <el-table-column
              align="center"
              header-align="center"
            ...
            >
              <template #default="{ row }">
                <el-button
                  ...
                  type="warning"
                  @click="
                    openPlatformOptionTranslateModal(
                      'option_item',
                      row.platform_store_good_code,
                      {
                        platform_good_option_no: row.platform_good_option_no,
                        origin_name: row.platform_store_good_option_name,
                        ko: row.platform_store_good_option_name_array.ko,
                        en: row.platform_store_good_option_name_array.en,
                        jp: row.platform_store_good_option_name_array.jp,
                        zh_hans:
                          row.platform_store_good_option_name_array.zh_hans,
                        zh_hant:
                          row.platform_store_good_option_name_array.zh_hant,
                      },
                    )
                  "
                >
    ...
    >
      ...
      >
        ...
        >
          <el-row align="middle">
            <span class="mr-10"> 옵션 그룹 이름•상품코드 검색 </span>
            <el-input
      ...
      >
        ...
        >
          <template #default="{ row }">
            {{
              `${convertToEllipsis(row.platform_store_good_display_name, 10)}
            (${row.platform_store_good_code})`
            }}
          </template>
        </el-table-column>
                ...
                openPlatformOptionTranslateModal(
                  ...
                  row.platform_store_good_code,
                  {
                    platform_good_option_group_no:
                      row.platform_good_option_group_no,
    ...
    >
      ...
      >
        ...
        <el-table-column
          ...
          header-align="center"
          label="옵션 상품 이름"
          prop="platform_store_good_option_name"
        />
        <el-table-column
          align="center"
          header-align="center"
                ...
                openPlatformOptionTranslateModal(
                  ...
                  row.platform_store_good_code,
                  {
                    ...
                    platform_store_good_option_code:
                      row.platform_store_good_option_code,
                    platform_store_good_option_name:
                      row.platform_store_good_option_name,
                    origin_name: row.platform_store_good_option_name,
                    ko: row.lang.ko,
                    en: row.lang.en,
                    jp: row.lang.jp,
