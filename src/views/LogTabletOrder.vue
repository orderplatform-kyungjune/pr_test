...
import { authentication, dateFormatterUtil, listTotalCountUtil, runtimeCheckHelper } from '@utils';
import { logTabletOrder } from '@router/routePaths';
import { requestTabletLogListParamType, tabletGetOrderLogDataType } from '@interface/logs';
import useExcelDownload from '@composables/excelDownload';
import { BreadcrumbHeader } from '@components';
import { LOG_TABLET_ORDER } from '@common/string';
import { logsCodec } from '@codecs';
import { endpoints, logs } from '@apis';

const { runtimeCheck } = runtimeCheckHelper;
const { failAuthenticationAlert } = authentication;
const { requestTabletOrderLogList } = logs;
const { refinedToday } = dateFormatterUtil;
const { isCountableList, getListTotalCount } = listTotalCountUtil;
const { tabletGetOrderLogCodec } = logsCodec;
...
// BreadcrumbHeader 프롭
const logTabletOrderHeaderProp = reactive([{ name: LOG_TABLET_ORDER }]);

// 상품 변경 로그 데이터 [서버 대기중]
const tabletOrderLogData: Ref<tabletGetOrderLogDataType[]> = ref(
  [] as tabletGetOrderLogDataType[],
);
...
const getTabletOrderLogList = async (page?: number) => {
  ...
  try {
    ...
    const res = (await requestTabletOrderLogList(searchWord)) as AxiosResponse;
    const typeError = runtimeCheck(tabletGetOrderLogCodec, res.data);
    ...
    if (!typeError) {
      if (res.status === 200) {
        tabletOrderLogData.value = res.data.data;
...
const setQueryData = async () => {
  ...
  pushPageWithQuery(logTabletOrder, searchData);
};
...
const excelFileHref: Ref<string> = ref(
  endpoints.excel.logs_tablet_get_order_download,
);

const getExcelTitle = () => {
  const todayDate = refinedToday().replaceAll('-', '').slice(2);
  return `tablet_order_${todayDate}`;
};
...
<template>
  <BreadcrumbHeader :propsHeader="logTabletOrderHeaderProp" />
  <el-card
    class="search-card"
    shadow="never"
  >
  ...
  <div class="tablet-log-table-container">
    ...
    >
      ...
      <el-table-column
        align="center"
        label="결과 소요 시간"
        prop="T_order_tablet_result_time"
        width="150px"
      />
      <el-table-column
        align="center"
